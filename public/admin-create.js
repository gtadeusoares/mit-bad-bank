function AdminCreate () {
    ...
    return (
        ...
        status={status}
        body={show ? 
            <AdminCreateForm setShow={setShow} setStatus={setStatus} /> : 
            <AdminCreateMsg setShow={setShow} setStatus={setStatus} />}
        />
    )
}
  
  function AdminCreateMsg(props){
    function logout() {
      loggedUser.email = '';
      loggedUser.balance = '';
      loggedUser.name = '';
    ...
    return(<>
      ...
      <button type="submit" 
        ...
        onClick={logout}><a href='/'>Logout</a></button>
    </>);
  }
  
  function AdminCreateForm(props){
    ...
    function handle (){
      ...
      fetch(`/account/create/${name}/${email}/${password}/admin`)
